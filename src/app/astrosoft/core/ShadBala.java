...
public class ShadBala  {
	...
	public ShadBala(PlanetaryInfo planetaryInfo, HousePosition housePosition,
		...
		birthDay.setTime(astroData.birthDay().getTime());

		if (astroData.birthTime() < sunrise) {
	...
	private EnumMap<Planet, Double> calcTribhagaBala() {
		...
		} else if (astroData.birthTime() < sunrise) {
		...
		} else if (astroData.birthTime() > sunset) {
		...
		if (birthDayNight) {
			...
			div = (int) ((astroData.birthTime() - lsunrise) / part) + 1;
			...
			double diff = astroData.birthTime() - lsunset;
	...
	private EnumMap<Planet, Double> calcHoraBala() {
		...
		double dur = astroData.birthTime() - sunrise;
	...
	private EnumMap<Planet, Double> calcNatonnataBala() {
		...
		double btimeDeg = SwissHelper.calcNatonnataBalaDeg(astroData.birthSD(),
				astroData.birthTime());
	...
	private EnumMap<Planet, Double> calcChestabala() {
		...
		double utime = astroData.birthTime()
		...
		// AstroUtil.dms(astroData.birthTime()) + " UT: " + AstroUtil.dms(utime)
		// );
